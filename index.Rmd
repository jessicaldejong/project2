...
heart_gower <- heart %>%
    select(Cholesterol, RestingBP, MaxHR, HeartDisease)
heart_gower$HeartDisease <- as.factor(heart_gower$HeartDisease)

gower1 <- daisy(heart_gower, metric = "gower")

silwidth <- vector()

for (i in 2:10) {
    pamfit <- pam(gower1, diss = TRUE, k = i)
...
I am using numeric variables cholesterol, max BP, and resting BP w binary value. The Gower medoid for the first cluster is ID #125, and the medoid for the second cluster is ID #531. The silhouette width is 0.69, which is an average of the gower values. 
...
Eigen values and variance percentages - optimal number of PCS to keep? I found the optimal number of PCs was two (found form eigen values) and the model accounts for 77% of the variance. 

When PC1 scores high, Cholesterol and Max HR are directly correlated. When choleseterol levels are igh, max HR is high. 
...
No concluding remarks. I enjoyed this semester. Thank you! (-:
